...
import ar.edu.uncuyo.gimnasio_sport.enums.*;
...
public class DataInitialization implements CommandLineRunner {
    ...
    private final TipoDePagoRepository formaDePagoRepository;
    ...
    private void crearFacturaDePrueba() {
        System.out.println("Creando factura de prueba...");

        // 1️⃣ Tomamos un socio existente (el primero, por ejemplo)
        Socio socio = socioService.listarSocios().stream()
                .filter(s -> !s.isEliminado())
                .findFirst()
                .orElseThrow(() -> new RuntimeException("No hay socios para generar factura de prueba"));

        // 2️⃣ Tomamos alguna cuota mensual de ese socio
        CuotaMensual cuota = cuotaMensualRepository.findAllBySocioIdAndEliminadoFalseOrderByFechaVencimientoDesc(socio.getId())
                .stream()
                .findFirst()
                .orElseThrow(() -> new RuntimeException("El socio no tiene cuotas para generar factura"));

        // 3️⃣ Creamos forma de pago
        FormaDePagoDto formaDePago = FormaDePagoDto.builder()
                .tipoDePago(TipoDePago.EFECTIVO)
                .observacion("Pago automático de prueba")
                .build();

        // 4️⃣ Creamos el detalle
        DetalleFacturaDto detalle = DetalleFacturaDto.builder()
                .cuotaMensualId(cuota.getId())
                .build();

        // 5️⃣ Armamos factura DTO
        FacturaDto facturaDto = FacturaDto.builder()
                .fechaFactura(LocalDate.now())
                .detalles(List.of(detalle))
                .totalPagado(cuota.getValorCuota().getValorCuota())
                .formaDePago(formaDePago)
                .estado(EstadoFactura.PAGADA)
                .build();

        // 6️⃣ Guardamos factura
        facturaService.crearFactura(facturaDto);

        System.out.println("Factura de prueba creada correctamente para el socio: " + socio.getNombre() + " " + socio.getApellido());
    }
